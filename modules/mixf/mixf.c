...
void module_init(void) {
  ...
  // initialize 1pole filters for audio processing
  ...
  param_setup(eParam_inSlew0, PARAM_SLEW_DEFAULT);
  param_setup(eParam_inSlew1, PARAM_SLEW_DEFAULT);
  param_setup(eParam_inSlew2, PARAM_SLEW_DEFAULT);
  param_setup(eParam_inSlew3, PARAM_SLEW_DEFAULT);
...
void module_process_frame(void) { 
  ...
  // take absolute value of in[0]
  inF[0] = abs_fr1x32( in[0] );

  // send adc into filters
  filter_1p_lo_in( &(inSlew[0]), inF[0] );
  filter_1p_lo_in( &(inSlew[1]), in[1] );
  filter_1p_lo_in( &(inSlew[2]), in[2] );
  filter_1p_lo_in( &(inSlew[3]), in[3] );

  // update audio filters at slew rate
  inF[0] = filter_1p_lo_next( &(inSlew[0]) );
  inF[1] = filter_1p_lo_next( &(inSlew[1]) );
  inF[2] = filter_1p_lo_next( &(inSlew[2]) );
  inF[3] = filter_1p_lo_next( &(inSlew[3]) );
  ...
  */
  ...
  outBus = add_fr1x32( outBus, mult_fr1x32x32(inF[0], adcVal[0]) );
  outBus = add_fr1x32( outBus, mult_fr1x32x32(inF[1], adcVal[1]) );
  outBus = add_fr1x32( outBus, mult_fr1x32x32(inF[2], adcVal[2]) );
  outBus = add_fr1x32( outBus, mult_fr1x32x32(inF[3], adcVal[3]) );
  */

  outBus = mult_fr1x32x32( inF[0], inF[1] );
